...
const SignupPage = () =>
 {
  const { loading: submitLoading, signUp } = useAuthStore();
  const {
    email,
    fullName,
    password,
...
(null);
  ...
  const [accountType, setAccountType] = useState<'user' | 'admin'>
('user');
  ...
  useEffect(() =>
 {
  ...
  const handleAccountTypeSelect = async (type: 'user' | 'admin') =>
 {
    try {
      setTransitionLoading(true);
      setAccountType(type);
      await router.replace({ query: { ...router.query, type } });
      setShowForm(true);
    } finally {
...
Join Our Platform</h1>

              <p className="text-gray-600">
Choose how you want to use our platform</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div 
                onClick={() =>
 !transitionLoading && handleAccountTypeSelect('admin')}
                className={`bg-white rounded-xl shadow-lg p-6 border border-indigo-100 transition-all duration-300 ${
                  transitionLoading ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-xl cursor-pointer'
                }`}
              >
...
List and manage your venues</p>
              ...
              <div 
                onClick={() =>
 !transitionLoading && handleAccountTypeSelect('user')}
                className={`bg-white rounded-xl shadow-lg p-6 border border-indigo-100 transition-all duration-300 ${
                  transitionLoading ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-xl cursor-pointer'
                }`}
              >
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-xl shadow-lg px-8 py-10 border border-indigo-100">
            ...
            <button
              ...
              disabled={transitionLoading || submitLoading}
              className={`group mb-6 flex items-center text-sm text-gray-600 hover:text-indigo-600 transition-colors ${
                (transitionLoading || submitLoading) ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >

              <svg className="h-4 w-4 mr-2 transition-transform group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
